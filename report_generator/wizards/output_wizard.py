...
class ReportGeneratorOutputWizard(models.TransientModel):
    _name = "reportgenerator.output.wizard"
    _description = "Wizard Report Generator"
    
    output_file_ids = fields.One2many(
        comodel_name="reportgenerator.output.file",
...
class ReportGeneratorOutputFile(models.TransientModel):
    _name = "reportgenerator.output.file"
    _description = "Report Generator Output File"
    
    output_id = fields.Many2one(
        comodel_name="reportgenerator.output.wizard",
        string="Output"
    )
