...
class ReportGeneratorReport(models.Model):
    _name = "reportgenerator.report"
    ...
    placeholder_ids = fields.One2many(
        comodel_name="reportgenerator.report.placeholder",
    ...
    def create(self, values):
        res = super(ReportGeneratorReport, self).create(values)
    ...
    def write(self, values):
        res = super(ReportGeneratorReport, self).write(values)
    ...
    def _validate_name_and_identificative(self):
        if self.env['reportgenerator.report'].search([('name', '=', self.name), ('id', '!=', self.id)]):
            raise ValidationError(_("A report with this name already exists, report's name need to be unique."))
        if self.env['reportgenerator.report'].search([('code', '=', self.code), ('id', '!=', self.id)]):
    ...
    def copy(self, default={}):
        ...
        return super(ReportGeneratorReport, self).copy(default)
    ...
    def return_output(self, files, wizard_name):
        ...
        for file in files:
            output.append((0, 0, {
                    'filename': file['name'],
                    'file': file['content'],
                    'download_btn': self.env['reportgenerator.output.file'].download_action(file['name'], file['content'])
                }))
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'reportgenerator.output.wizard',
            'target': 'new',
            'context': {
    ...
    def get_report(self, code):
        return self.env['reportgenerator.report'].search([('code', '=', code)], limit=1)
    ...
    def generate_report(self, res_id):
        ...
        try:
            for page in range(existing_pdf.numPages):
                ...
                placeholders = self.env['reportgenerator.report.placeholder'].search([('id','in',self.placeholder_ids.ids), ('page','=',page+1)])
                ...
                for placeholder in placeholders:
                    ...
                    can.setFillColor(HexColor(str(placeholder.text_color)))
    ...
    def preview_report(self):
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'reportgenerator.preview.wizard',
            'target': 'new',
            'context': {
    ...
    def action_list_action_server(self):
        return {
            ...
            'target': 'current',
            'domain': [('reportgenerator_report_id', '=', self.id)],
    ...
    def _default_action_server_values(self):
        default_code = """
reportgenerator = env['reportgenerator.report'].sudo()
report = reportgenerator.get_report('%s')
reports = []

for record in records:
    reports.append(report.generate_report(record.id))

action = reportgenerator.return_output(reports, 'Documents')

""" % (self.code)

        return {
            'default_reportgenerator_report_id': self.id,
    ...
    def _compute_ir_action_server_count(self):
        for record in self:
            record.ir_action_server_count = self.env['ir.actions.server'].sudo().search([('reportgenerator_report_id', '=', record.id)], count=True)